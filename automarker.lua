...
do
    ...
    function element_proto:UpdateManuallyAssigned()
        ...
        self:print("group", IsInGroup(), "raid", IsInRaid())
        if IsInRaid() then
...
function element_proto:AssignRaidIcon(event, data)
    ...
    if iconIndex then
        ...
        if self:IsInCombat(unit, guid) then
            self:SetRaidTarget(data, iconIndex)
        elseif not self.queue[guid] then
...
function element_proto:FilterData(data)
    return data and data.npcID and CanBeRaidTarget(data.unit) and (not data.ignore)
end
...
function element_proto:ProcessData(unit)
    if UnitExists(unit) then
        ...
        local mobInfo = LibMobs:GetMob(guidInfo.instanceID, guidInfo.npcID) or { priority = 0, ignore = true }

        return Mixin({ unit = unit, guid = guid, name = name }, guidInfo, mobInfo)
    end
    return nil
end
...
function element_proto:Update(event, unit)
    ...
    self:print("queue", #self.queue)
end

function element_proto:Resume()
    -- self:print("units", self:table_length(self.units))
    -- self:print("guids", self:table_length(self.guids))
    -- self:print("enemies", self:table_length(self.enemies))
    -- self:print("nameplates", self:table_length(self.nameplates))
    -- self:print("assignedIcons", self:table_length(self.assignedIcons))
    -- self:print("manuallyAssignedIcons", self:table_length(self.manuallyAssignedIcons))
    for iconIndex = 1, NUM_RAID_ICONS do
        local data = self.assignedIcons[iconIndex]
        self:print(iconIndex, RAID_ICONS[iconIndex], data and data.unit, data and data.guid, self.manuallyAssignedIcons[iconIndex])
    end
end
...
function element_proto:COMBAT_LOG_EVENT_UNFILTERED()
    ...
    if subevent == "UNIT_DIED" and destGUID then
        ...
        self:print(subevent, "guid", destGUID, "unit", unit)
...
function element_proto:OnUpdate(elapsed)
    ...
    if self.interval <= 0 then
        ...
        for iconIndex = length, 1, -1 do
            local data = self.queue[iconIndex]
            table.remove(self.queue, iconIndex)
            self.queue[data.guid] = nil
